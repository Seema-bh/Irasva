...
(function ($) {
  ...
  var ella = {
    ...
    checkItemsInDropdownCart: function () {
      ...
      if (miniProductList.children().length) {
        cartHasItems.show();
        cartsubtotal.css("visibility", "visible");
        cartNoItems.hide();
      } else {
        cartHasItems.hide();
        cartsubtotal.css("visibility", "hidden");
        cartNoItems.show();
      };
    },
    ...
    removeItemDropdownCart: function (cart) {
      // Remove comment
      var btnRemove = dropdownCart.find('.btn-reove');
    ...
    doUpdateDropdownCart: function (cart) {
      ...
      if(cart.items.length>0){
        $('[data-cart-count]').text(cart.items.length).css("display","flex");
      }else{
        $('[data-cart-count]').text(cart.items.length).css("display","none");
      }
  ...
  //     <input type="text" class="custom_qty"  value="{{ custom_quantity }}">
  ...
  // </div>
      if (cart.item_count > 0) {
        for (var i = 0; i < cart.items.length; i++) {
          var pricee = cart.items[i].line_price;
          var qtty = cart.items[i].properties.quantity;
          if(qtty == undefined){
            qtty = cart.items[i].quantity;
          }
          console.log("---qtty---",qtty);
          var template = '<li class="item test4" id="cart-item-{ID}"><a href="{URL}" title="{TITLE}" class="product-image"><img src="{IMAGE}" alt="{TITLE}"></a><div class="product-details"><div class="pro-name-and-price"><a class="product-name" href="{URL}">{TITLE}</a></div><div class="price_quntity_box"><div class="cart-collateral"><div class="quantity-data">';
          // template += '<div class="option quantity-box text-center"><div class="dec button">-</div><input type="text" class="custom_qty quantity"  value="'+ qtty +'"><div class="inc button">+</div></div>';
          // template += ' <div class="qty-group"> <a class="minus button" data-minus-quantity href="#"></a>';
          // template += '<input type="text" class="custom_qty quantity"  value="'+ qtty +'">'; 
          // template += '<input type="hidden" class="custom_price" name="updates[]" item_id="'+cart.items[i].id+'" id="updates_'+cart.items[i].id+'" value="'+ cart.items[i].quantity +'" pro="';
          // for (p in cart.items[i].properties) {
          //   template +=  p + ":" + cart.items[i].properties[p]+",";
          // }
          // template += '"test line_item="'+k+'">';
          // template += ' <a class="plus button" data-plus-quantity href="#"></a></div>';
          template += '</div><span class="price">';
          template += Shopify.formatMoney(pricee , window.money_format);
          template += '</span></div>';
          template += '<a href="/cart/change?line=';
          template += k; 
          template += '&amp;quantity=0" title="{{ "cart.general.remove" | t }}" class="btn-remove"><svg aria-hidden="true" data-prefix="fal" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" class="svg-inline--fa fa-times fa-w-10 fa-2x"><path fill="currentColor" d="M193.94 256L296.5 153.44l21.15-21.15c3.12-3.12 3.12-8.19 0-11.31l-22.63-22.63c-3.12-3.12-8.19-3.12-11.31 0L160 222.06 36.29 98.34c-3.12-3.12-8.19-3.12-11.31 0L2.34 120.97c-3.12 3.12-3.12 8.19 0 11.31L126.06 256 2.34 379.71c-3.12 3.12-3.12 8.19 0 11.31l22.63 22.63c3.12 3.12 8.19 3.12 11.31 0L160 289.94 262.56 392.5l21.15 21.15c3.12 3.12 8.19 3.12 11.31 0l22.63-22.63c3.12-3.12 3.12-8.19 0-11.31L193.94 256z" class=""></path></svg></a></div></div></li>';
          var item = template;
          $.ajax({
    ...
    changeQuantity: function () {
      var count = 0;
      $("body,html").on("click", ".item .button", function (i) {
        count = count + 1;
        console.log(count);
        if (count % 2 != 0) {
          ...
          $.ajax({
            ...
            success: function (i) {
              ella.hideLoading(), ella.updateDropdownCart()
    ...
    initNewsLetterPopup: function () {
      if (window.newsletter_popup) {
        ...
        if (Cookies.get('emailSubcribeModal') != 'closed') {
          ella.ellaTimeout = setTimeout(function () {
        ...
        $(".wrapper-newsletter .column-left").on('click', function (e) {
            ella.closeEmailModalWindow(newsletterWrapper);
        });
    ...
    changeQuantityAddToCart: function () {
      ...
      doc.on('click', buttonSlt, function (e) {
        ...
        var self = $(this),
            ...
            var custom_price = self.siblings(".custom_price").val(),
            custom_price = custom_price.replace(".00","").replace("₹ ","").replace(",","");
            console.log("hihihijijijijijjiji",custom_price);


        if (input.length < 1) {
          input = self.siblings('input.custom_qty');
        }
          var pro = self.siblings(".custom_price").attr("pro");
          var line_item = self.siblings(".custom_price").attr("line_item");
        var val = parseInt(input.val());
        var cal_custm_price = parseInt(custom_price / val) ;
        console.log("cal_custm_price",custom_price,"/",val,"=pro",pro,cal_custm_price);
        switch (true) {
          case (self.hasClass('plus')):
            {
              val += 1;
              console.log("cal_custm_price",cal_custm_price,"*",val,"=",cal_custm_price * val);
              var update_price = val * cal_custm_price;
              self.siblings(".custom_price").val(val * cal_custm_price);
              if(pro != undefined){
              ella.qty_updatecart(val,update_price,pro,line_item);
              }
              break;
            }
    ...
    qty_updatecart: function (val,update_price,pro,line_item) {
        console.log("val---",val,"wowowo",update_price); 
        var abc =  pro.split(","),
        qty = update_price;
    ...
    var data = {
      ...
      console.log("abc---==",abc);
      for (var i in abc){
       console.log(abc[i],"Hey");
       var afg = abc[i].split(':'),
       afgone = afg[0],
       afgtwo = afg[1];
       if(afgone == "quantity"){
           console.log("koool");
           tempProperties["quantity"] = val;
       }else{
    ...
    initAddToCart: function () {
      var btnAddToCartSlt = '[data-btn-addToCart]',
          btnAddToCartNoCustom = '[data-btn-addToCart-no-custom]',
          buyNowNoCustom = '[data-buy-now-no-customization]',
          buyNowCustom = '[data-buy-now-button]';
      ...
      if (localStorage.getItem("currentPincode") != null && localStorage.getItem("currentPincode") != undefined) {
        ...
        jQuery(".availability-text").html("<span style='color:green; padding: 8px 0 0;font-weight: 500;'>Shipping available! Order will be delivered within 5-6 working days.</span>").fadeIn(100);
      }
      ...
      $('#check-delivery-submit').click(function (e) {
        ...
        if (buttonText == "Check") {
          ...
          if (cod != -1) {
            ...
            jQuery(".availability-text").html("<span style='color:red; padding: 8px 0 0;'>Wrong pincode</span>").fadeIn(100);
      ...
      //   Buy now no custom product   
      doc.off('click.addToCart', buyNowNoCustom).on('click.addToCart', buyNowNoCustom, function (e) {
        e.preventDefault();
        e.stopPropagation();
        console.log("hi hi");
        var self = $(this);
        ...
        if (self.attr('disabled') !== "disabled") {
          ...
          var var_id = self.attr("var_id");
          var quantity = $('.quantity [name="quantity"]').val();
          var form = productItem.find('form');
          var handle = productItem.find('.product-grid-image').data('collections-related') || sectionsProduct.data('collections-related');
          ...
          ella.doAjaxBuyNow(quantity,data,var_id, title, image);

        }
        return false;
      });
      
      //    Buy now custom product
      doc.off('click.addToCart', buyNowCustom).on('click.addToCart', buyNowCustom, function (e) {
        e.preventDefault();
        e.stopPropagation();
        console.log("hi Irasssva");
        var self = $(this);
        var thisForm = $(self.data('form-id'));
        ...
        if (self.attr('disabled') !== "disabled") {
          ...
          ella.doAjaxBuyNowNormal(data, title, image);
      ...
      doc.off('click.addToCart', btnAddToCartNoCustom).on('click.addToCart', btnAddToCartNoCustom, function (e) {
        e.preventDefault();
        e.stopPropagation();
        console.log("hi hi");
        var self = $(this);
        ...
        if (self.attr('disabled') !== "disabled") {
          ...
          ella.doAjaxAddToCart(quantity,data,var_id, title, image);
    ...
    doAjaxAddToCartNormal: function (data, title, image) {
      console.log("Check Irasva Hihihi",data,"hihhiihi");
      var data = data.split('&quantity=');
      console.log("jijij",$("#product-add-to-cart").attr("var_id"),"ENGRAVING",$("#compalsury-name").val());
      var carat = $( "#metal option:selected" ).val(),
          timestamp = Date.now(),
          size = $( "#select_size option:selected" ).val(),
//           engraving = $("#compalsury-name").val(),
          diamond_type = $("#diamond_type").attr("type"),
          quantity = $(".price_cal").attr("price"),  
          qty = 1; 
      console.log("quantity",quantity);
      var data = {
        quantity: quantity,
        id: $("#product-add-to-cart").attr("var_id"),
        properties: {
      ...
      // var new_qty = $('.price_cal').html();     
      // new_qty = new_qty.replace('₹ ','');
      // var data = data[0] +"&quantity="+ new_qty;
    ...
    doAjaxAddToCart: function (quantity, data, var_id , handle, nojax) {
      ...
      console.log("datatatata",var_id,data);
      $.ajax({
        type: "POST",
        url: "/cart/add.js",
        data: data, 
    ...
    doAjaxBuyNowNormal: function (data, title, image) {
      console.log("Check Irasva Hihihi",data,"hihhiihi");
      var data = data.split('&quantity=');
      console.log("jijij",$("#product-add-to-cart").attr("var_id"),"ENGRAVING",$("#compalsury-name").val());
      var carat = $( "#metal option:selected" ).val(),
...
//           engraving = $("#compalsury-name").val(),
    ...
    doAjaxBuyNow: function (quantity, data, var_id , handle, nojax) {
      if(quantity != undefined){
        var data = {
          quantity: quantity,
          id: var_id
  ...
  var iWishCountNumber = $(".wishlist-icon .iWishCount").text();
  var cartCountNumber = $.trim($(".header-mb .cartCount").text());
  console.log(cartCountNumber,iWishCountNumber);
  if(iWishCountNumber != "00"){
    $(".iWishCount").css("display","flex");
  }
  ...
  $(".videoConsultation").on("click",function(){
    var videoSku = $(this).attr("data-sku");
    var productTitle = $(this).attr("data-title");
    localStorage.setItem("videoProduct",productTitle + " - " + videoSku );
    window.location.href = "/pages/video-consultation";
  });
  $(".try-at-home-nav,.mobile-video-consultation .icon-try,.video-consult-mobile a").on("click",function(){
    localStorage.removeItem("videoProduct");
  });
  $(".mobile-video-consultation .icon-try").on("click",function(){
    localStorage.removeItem("videoProduct");
  });

  var pageUrl = window.location.href;
  if(pageUrl.indexOf("try-at-home")>-1){
    var try_it = Cookies.get("try_it");
    var json = JSON.parse(try_it);
    console.log(json, "try_it_2.1", try_it);
    console.log("json.objects.length",json.objects.length);
    $(".try_count").html(json.objects.length);
  }
